...
const App = () => {
  ...
  useEffect(() => {
    ...
    const storedPendingData = JSON.parse(localStorage.getItem('pendingSyncData')) || {
      sales: [], orders: [], inventoryProducts: [], inventorySupplies: [], logisticsRoutes: []
    };
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'finance':
        return <FinanceModule currentUserRole={currentUserRole} isOffline={isOffline} salesData={salesData} invoicesData={[]} />; // invoicesData needs to be passed from App
      case 'orders':
      ...
      case 'invoices':
        return <InvoiceModule currentUserRole={currentUserRole} isOffline={isOffline} />;
      case 'clients':
