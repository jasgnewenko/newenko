...
const InventoryModule = ({ garrafones, setGarrafones, insumos, setInsumos, currentUserRole, isOffline }) => {
  ...
  const handleUpdateStock = (type, id, value) => {
    if (currentUserRole === 'Bodeguero' || currentUserRole === 'Administrador') {
      if (type === 'garrafones') {
      ...
      alert('No tienes permiso para actualizar el inventario.');
    }
  };
  ...
  const handleManualStockChange = (type, id, field, value) => {
    if (currentUserRole === 'Bodeguero' || currentUserRole === 'Administrador') {
      ...
      alert('No tienes permiso para modificar el inventario manualmente.');
    }
  };

  const handleAddProduct = (e) => {
    e.preventDefault();
    if (currentUserRole === 'Administrador') { // Solo administradores pueden agregar nuevos tipos de productos/insumos
      ...
      alert('No tienes permiso para agregar nuevos productos.');
    }
  };
  ...
  const handleAddSupply = (e) => {
    ...
    if (currentUserRole === 'Administrador') { // Solo administradores pueden agregar nuevos tipos de productos/insumos
      ...
      alert('No tienes permiso para agregar nuevos insumos.');
    }
  };
  ...
  const gptInventoryAssistant = "¡Hola! Soy tu Encargado de Inventario. Aquí puedes controlar el stock de tus productos y los insumos. Asegúrate de mantener los niveles adecuados para no quedarte sin nada. Si ves que algo está bajo, ¡es momento de reabastecer!";

  return (
    <section className="p-6 bg-gray-800 rounded-2xl shadow-lg mt-6">
      <h2 className="text-2xl font-bold text-gray-100 mb-6">Módulo de Inventario</h2>

      <div className="bg-gray-900 p-4 rounded-lg mb-6 text-gray-300 text-sm italic">
        <p>{gptInventoryAssistant}</p>
      </div>
        ...
        <>
          ...
          <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-4 bg-gray-900 rounded-lg shadow-inner">
            ...
            <div className="md:col-span-3">
              ...
              {isOffline && <p className="text-red-400 text-sm mt-2 text-center">No puedes agregar productos en modo offline.</p>}
            </div>
          </form>
        </>
      )}
              ...
              <tr key={item.id} className="border-b border-gray-700 hover:bg-gray-700 transition-colors duration-200">
                ...
                <td className="py-3 px-6 text-left">
                  <input
                    ...
                    disabled={isOffline || (currentUserRole !== 'Bodeguero' && currentUserRole !== 'Administrador')}
                  />
                </td>
                <td className="py-3 px-6 text-left">
                  <input
                ...
                <td className="py-3 px-6 text-center">
                  <button onClick={() => handleUpdateStock('garrafones', item.id, 1)} className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-md mr-2 transition-colors" disabled={isOffline || (currentUserRole !== 'Bodeguero' && currentUserRole !== 'Administrador')}>+</button>
                  <button onClick={() => handleUpdateStock('garrafones', item.id, -1)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md transition-colors" disabled={isOffline || (currentUserRole !== 'Bodeguero' && currentUserRole !== 'Administrador')}>-</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <>
          ...
          <form onSubmit={handleAddSupply} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-4 bg-gray-900 rounded-lg shadow-inner">
            ...
            <div className="md:col-span-3">
              ...
              {isOffline && <p className="text-red-400 text-sm mt-2 text-center">No puedes agregar insumos en modo offline.</p>}
            </div>
          </form>
        </>
      )}
          ...
          <tbody className="text-gray-200 text-sm font-light">
            {insumos.map(item => (
              <tr key={item.id} className="border-b border-gray-700 hover:bg-gray-700 transition-colors duration-200">
                ...
                <td className="py-3 px-6 text-center">
                  <button onClick={() => handleUpdateStock('insumos', item.id, 1)} className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-md mr-2 transition-colors" disabled={isOffline || (currentUserRole !== 'Bodeguero' && currentUserRole !== 'Administrador')}>+</button>
                  <button onClick={() => handleUpdateStock('insumos', item.id, -1)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md transition-colors" disabled={isOffline || (currentUserRole !== 'Bodeguero' && currentUserRole !== 'Administrador')}>-</button>
                </td>
              </tr>
            ))}
          </tbody>
